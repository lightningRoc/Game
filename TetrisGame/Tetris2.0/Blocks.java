...
public class Blocks {
	...
	private int type;//标识下落块的类型
	private int direction;//标识下落块的方向
	...
	//根据给定坐标与方向给出各种块
	public Block[] getBlocks1(int x,int y,int direct)
	{
		if(x==5)x+=10;
		if(x==495)x-=10;
		if(direct==1)return new Block[] {new Block(x,y),new Block(x+10,y),new Block(x,y+10),new Block(x-10,y+10)};
		else return new Block[] {new Block(x,y),new Block(x,y+10),new Block(x-10,y),new Block(x-10,y-10)};
	}
	
	public Block[] getBlocks2(int x,int y,int direct)
	{
		if(x==5)x+=10;
		return new Block[] {new Block(x,y),new Block(x-10,y),new Block(x,y+10),new Block(x-10,y+10)};
	}
	
	public Block[] getBlocks3(int x,int y,int direct)
	{
		if(x==5)x+=10;
		if(direct==1)return new Block[] {new Block(x,y),new Block(x,y+10),new Block(x,y-10)};
		else return new Block[] {new Block(x,y),new Block(x+10,y),new Block(x-10,y)};
	}
	
	public Block[] getBlocks4(int x,int y,int direct)
	{
		if(x==5)x+=10;
		if(x==495)x-=10;
		if(direct==1)return new Block[] {new Block(x,y),new Block(x-10,y),new Block(x,y+10),new Block(x+10,y+10)};
	...
	public boolean getNewBlocks()
	{
		...
		int getNum=rand.nextInt(4);//随机获取新的块样式
		switch(getNum)
		{
		case 0:
			currentBlocks=getBlocks1(newX,5,direction);
			type=1;
			break;
		case 1:
			currentBlocks=getBlocks2(newX,5,direction);
			type=2;
			break;
		case 2:
			currentBlocks=getBlocks3(newX,5,direction);
			type=3;
			break;
		case 3:
			currentBlocks=getBlocks4(newX,5,direction);
			type=4;
			break;
		}
	...
	{
		int newX=currentBlocks[0].getX();
		int newY=currentBlocks[0].getY();
		Block[] auxBlocks=currentBlocks;
		direction=-direction;
		switch(type)
		{
		case 1:
			currentBlocks=getBlocks1(newX,newY,direction);
			break;
		case 2:
			currentBlocks=getBlocks2(newX,newY,direction);
			break;
		case 3:
			currentBlocks=getBlocks3(newX,newY,direction);
			break;
		case 4:
			currentBlocks=getBlocks4(newX,newY,direction);
			break;
		}
		if(!canRotate())currentBlocks=auxBlocks;
	}
	...
	public boolean canRotate()
	{
		for(Block b:currentBlocks)
		{
			if(blocks.contains(new Block(b.getX(),b.getY())))//是否碰到其他块
			{
				return false;
			}
			
			if(b.getY()>TetrisGameGUI.getH()||b.getY()<0)return false;//是出边缘
			if(b.getX()>TetrisGameGUI.getW()||b.getX()<0)return false;
		}
		return true;
	}
